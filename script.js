...
function createLetters(text, rad, offset, frontColor, direction, id) {
    ...
    letters.forEach((letter, index) => {
        ...
        const rotateZ = direction === 'backwards' ? '0deg' : '180deg';
        letterElement.style.transform = `translate(-50%, -50%) translate(${x}px, ${y}px) rotateX(90deg) rotateY(${angle + 90}deg) rotateZ(${rotateZ}) translateY(${offset}px)`;
